"""
fetch_mlb.py
Fetches MLB schedules using ESPN's JSON API.
Mirrors the structure of fetch_nba.py / fetch_nhl.py.
Writes output to mlb.json.
...
WINDOW_HOURS = 5  # Baseball games can run long

ESPN_MLB_URL = (
    "https://site.api.espn.com/apis/site/v2/sports/baseball"
    "/mlb/scoreboard?dates={date_str}"
)

# National broadcasters only — regional team networks excluded
ESPN_SOURCE_MAP = {
    "ESPN":        "ESPN",
    "ESPN2":       "ESPN2",
    "ESPN+":       "ESPN+",
...
REGIONAL_EXCLUDE = {
    ...
    "root sports", "altitude", "fox sports", "space city", "sportsnet",
    "masn", "chsn", "marquee", "apple tv+",
    ".tv",  # team-specific streams like Cardinals.TV, Rays.TV etc
}
...
def fetch_mlb_day(date_str: str) -> list:
    url = ESPN_MLB_URL.format(date_str=date_str)
    try:
        data = fetch_json(url)
...
def main():
    ...
    with open("mlb.json", "w") as f:
        json.dump(output, f, indent=2)

    total = sum(len(d["games"]) for d in ordered.values())
    print(f"\nDone! {total} games written to mlb.json")
