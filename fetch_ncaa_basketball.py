...
def fetch_ncaa_day(date_str: str) -> list:
    ...
    for event in data.get("events", []):
        try:
            ...
            # DEBUG: print raw notes from both event and competition
            if event.get("season", {}).get("type") == 3:
                print(f"  [DEBUG] {event.get('name')}")
                print(f"          event.notes={event.get('notes')}")
                print(f"          competition.notes={competition.get('notes')}")
