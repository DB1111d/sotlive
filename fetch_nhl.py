"""
fetch_nhl.py
Fetches NHL schedules using ESPN's JSON API.
Mirrors the structure of fetch_ncaa_basketball.py.
Writes output to nhl.json.
"""
...
REGIONAL_EXCLUDE = {
    "msg", "nesn", "nbc sports", "bally", "root sports", "altitude",
    "fox sports", "sportsnet", "tva sports", "rds", "yes", "chsn",
    "fanduel", "spectrum",
}
...
def fetch_nhl_day(date_str: str) -> list:
    url = ESPN_NHL_URL.format(date_str=date_str)
    try:
        data = fetch_json(url)
    except Exception as e:
        print(f"    API error for NHL on {date_str}: {e}")
        return []
...
def main():
    ...
    with open("nhl.json", "w") as f:
        json.dump(output, f, indent=2)

    total = sum(len(d["games"]) for d in ordered.values())
    print(f"\nDone! {total} games written to nhl.json")
